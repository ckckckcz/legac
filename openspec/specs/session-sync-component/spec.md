...
## Summary
A zero-render client component (`SessionSync`) mounted inside `AuthSessionProvider` that calls `useSessionWithStorage()` to ensure localStorage is populated with session data and JWT token for every authenticated page in the application.

## Requirements

### Session sync component mounts at app root
